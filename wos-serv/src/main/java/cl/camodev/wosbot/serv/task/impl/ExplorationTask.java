...
public class ExplorationTask extends DelayedTask {
	...
	private final static String TASK_NAME = "Exploration Task";

	public ExplorationTask(DTOProfiles list, LocalDateTime scheduledTime) {
	...
	protected void execute() {
		ServLogs.getServices().appendLog(EnumTpMessageSeverity.INFO, TASK_NAME, profile.getName(), "Starting Task");
		...
		if (homeResult.isFound() || worldResult.isFound()) {
			ServLogs.getServices().appendLog(EnumTpMessageSeverity.INFO, TASK_NAME, profile.getName(), "going to exploration");
			...
			if (claimResult.isFound()) {
				ServLogs.getServices().appendLog(EnumTpMessageSeverity.INFO, TASK_NAME, profile.getName(), "claiming rewards");
