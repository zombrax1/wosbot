...
public class ConsoleLogLayoutController {
	...
	private static final Logger logger = Logger.getLogger(ConsoleLogLayoutController.class.getName());
	...
	private void initializeProfileFilter() {
		...
		try {
			...
			if (profiles != null) {
				...
				logger.warning("Profile list is null");
			}
		} catch (Exception e) {
	...
	public void appendMessage(DTOLogMessage dtoMessage) {
		// WARNING: Ensure dtoMessage.getMessage() does not contain sensitive data before logging/displaying.
